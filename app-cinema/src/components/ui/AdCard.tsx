...
const AdCard = ({ ad, delay = 0 }: Props) => {
  ...
  >
      ...
      >
        {esImagen ? (
          ...
          <div style={{ textAlign: 'center', padding: '.75rem' }}>
            <div style={{ fontSize: '.7rem', letterSpacing: '.12em', textTransform: 'uppercase', color: '#bfdbfe' }}>
              {tipo}
            </div>
            <div style={{ fontSize: '.9rem', fontWeight: 600, color: '#f1f5f9', marginTop: '.25rem' }}>
              {ad.activo ? 'Activo' : 'Inactivo'}
            </div>
            {esVideo && (
              <div style={{ fontSize: '.65rem', color: '#93c5fd', marginTop: '.25rem' }}>
      ...
      <div style={{ fontSize: '.72rem', fontWeight: 600, color: '#f1f5f9', lineHeight: 1.3 }}>
        {ad.titulo}
      </div>
      <div style={{ fontSize: '.62rem', color: '#94a3b8', marginTop: '.2rem', lineHeight: 1.4 }}>
        {ad.contenidoTexto}
      </div>
      {esVideo && ad.videoUrl && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          style={{ fontSize: '.62rem', fontWeight: 600, color: 'var(--accent2)', marginTop: '.35rem', display: 'inline-block' }}
        >
          ▶ Ver video
        </a>
      )}
