import { authPrivateClient, authPublicClient, } from '../../lib/httpClient'
...
export const authService = {
  login: async (username: string, password: string): Promise<Autentication> => {
    const { data } = await authPublicClient.post('/v1/users/auth/login', { username: username, password: password })
    return data
  },

  register: async (payload: UsuarioComunRequest): Promise<UsuarioComunResponse> => {
    const { data } = await authPublicClient.post('/v1/users/registration', payload)
    return data
  },

  verify: async (username: string, token: string): Promise<Autentication> => {
    const { data } = await authPublicClient.post('/v1/users/auth/verify', { username, token })
    return data
  },

  resetPassword: async (email: string): Promise<{ message: string }> => {
    const { data } = await authPrivateClient.post('/v1/users/me/credentials', { email })
    return data
  },

  confirmResetPassword: async (token: string, newPassword: string): Promise<{ message: string }> => {
    const { data } = await authPrivateClient.put('/v1/users/me/credentials', { token, newPassword })
    return data
  },

  updateProfile: async (idUsuario: number, payload: UpdateProfileRequest): Promise<UsuarioComunResponse> => {
    const { data } = await authPrivateClient.put(`/v1/users/me/${idUsuario}`, payload)
    return data
  },

  getProfile: async (username: string): Promise<UsuarioComunResponse> => {
    const { data } = await authPrivateClient.get(`/v1/users/me/${username}`)
    return data
  },
  ...
  getWallet: async (idUsuario: number): Promise<CarteraResponse> => {
    const { data } = await authPrivateClient.get(`/v1/users/me/wallets/${idUsuario}`)
    return data
  },

  updateWallet: async (idUsuario: number, operacion: 'ADD' | 'REMOVE', monto: number): Promise<CarteraResponse> => {
    const { data } = await authPrivateClient.post(`/v1/users/me/wallets/${idUsuario}`, { operacion, monto })
    return data
  },


  getAllRoles: async (): Promise<RolDTO[]> => {
    const { data } = await authPrivateClient.get('/v1/users/admin/roles')
    return data
  },

  asignarRol: async (idUsuario: number, idRol: number): Promise<UsuarioComunResponse> => {
    const { data } = await authPrivateClient.post(`/v1/users/admin/users/${idUsuario}/roles/${idRol}`)
    return data
  },
  ...
  getAllUsers: async (activo?: boolean, rol?: string): Promise<UsuarioComunResponse[]> => {
    ...
    const { data } = await authPrivateClient.get('/v1/users/admin/users', { params })
    return data
  },
