      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr .8fr', gap: '1rem' }}>
        <div style={{ borderRadius: '16px', padding: '1.5rem', background: 'rgba(30,64,175,0.12)', border: '1px solid rgba(96,165,250,0.15)' }}>
          ...
          <button
            ...
            style={{ padding: '.6rem 1rem', borderRadius: '10px', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, var(--blue-mid), var(--blue-light))', color: '#fff' }}
          >
        ...
        <div style={{ borderRadius: '16px', padding: '1.5rem', background: 'rgba(15,23,42,0.35)', border: '1px solid rgba(96,165,250,0.15)' }}>
          ...
          <div style={{ color: '#94a3b8', fontSize: '.85rem' }}>
            ...
            <div style={{ marginTop: '.5rem', display: 'grid', gap: '.5rem' }}>
              {actores?.length ? actores.map((actor, index) => (
                <div key={`${actor.actor}-${index}`} style={{ padding: '.65rem .75rem', borderRadius: '10px', background: 'rgba(30,64,175,0.12)' }}>
                  <div style={{ color: '#f1f5f9' }}>{actor.actor}</div>
                  <div style={{ fontSize: '.75rem' }}>Personaje: {actor.personaje}</div>
                </div>
              )) : (
                <div style={{ marginTop: '.5rem' }}>Sin reparto registrado.</div>
              )}
            </div>
          </div>
        </div>
