...
import type { ComentarioPeliculaRequest, ComentarioPeliculaResponse, PeliculaPostersResponse } from '../../types/CinemaCore.types'
...
const PeliculaDetallePage = () => {
  ...
  const [editingComment, setEditingComment] = useState<ComentarioPeliculaResponse | null>(null)
  ...
  const createMutation = useMutation({
    mutationFn: (payload: ComentarioPeliculaRequest) => cinemaUsuarioService.crearComentarioPelicula(peliculaId, payload),
    onSuccess: res => {
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: ComentarioPeliculaRequest) => cinemaUsuarioService.actualizarComentarioPelicula(editingComment!.idComentarioPelicula, payload),
    onSuccess: res => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (comentario: ComentarioPeliculaResponse) => cinemaUsuarioService.eliminarComentarioPelicula(comentario.idComentarioPelicula, auth!.idUsuario),
    onSuccess: res => {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ borderRadius: '16px', padding: '1.5rem', background: 'rgba(15,23,42,0.35)', border: '1px solid rgba(96,165,250,0.15)' }}>
            ...
            {comentariosLoading ? (
              ...
              <div style={{ color: '#94a3b8', fontSize: '.85rem' }}>Todavía no hay comentarios para esta película.</div>
            ) : (
              <div style={{ display: 'grid', gap: '.75rem' }}>
                {comentarios.map(comentario => {
                  const esMio = comentario.idUsuario === auth?.idUsuario
                  return (
                    <div key={comentario.idComentarioPelicula} style={{ borderRadius: '12px', padding: '1rem', background: 'rgba(30,64,175,0.12)', border: '1px solid rgba(96,165,250,0.12)' }}>
