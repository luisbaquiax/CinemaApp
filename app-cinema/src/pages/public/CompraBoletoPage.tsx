...
const CompraBoletoPage = () => {
  ...
  const { data: asientos = [], isLoading: asientosLoading } = useQuery({
    ...
    queryFn: () =>
      cinemaService.getAsientosDisponiblesOcupadosPorFuncion(Number(funcionId)),
    enabled: !!funcionId,
  });

  const asientosPorFila = useMemo(() => {
    return asientos.reduce<Record<string, AsientoResponse[]>>(
      (acc, asiento) => {
  ...
  const asientoSeleccionado = useMemo(
    () =>
      asientos.find((asiento) => asiento.idAsiento === asientoSeleccionadoId) ??
      null,
  ...
  useEffect(() => {
    ...
    setAsientoSeleccionadoId((prev) => {
      if (
        prev &&
        asientos.some(
          (asiento) => asiento.idAsiento === prev && asiento.disponible,
        )
      )
        return prev;
      return null;
  ...
  const comprarMutation = useMutation({
    ...
    onSuccess: (res) => {
      console.log("Boleto comprado:", res);
    ...
    onError: (err: any) =>
      setMsg({
        type: "err",
        text: err?.response?.data?.message || "No se pudo comprar el boleto.",
      }),
  });
  ...
  return (
    ...
    >
      ...
      <p
        style={{ color: "#94a3b8", fontSize: ".85rem", marginBottom: "1.5rem" }}
      >
        Elige un asiento disponible y completa la fecha de pago antes de
        confirmar.
      </p>
      ...
      >
            ...
            {peliculaLoading ? (
              ...
              <>
                ...
                <div style={{ color: "#94a3b8", fontSize: ".85rem" }}>
                  {cineSeleccionado?.nombreCompania ?? "Cine"} ·{" "}
                  {salaSeleccionada?.nombre ?? "Sala"}
                </div>
              </>
            )}
          </div>
                  ...
                  >
                    ...
                    >
                      {asientosFila.map((asiento) => {
                        const isSelected =
                          asientoSeleccionadoId === asiento.idAsiento;
                        return (
                          <button
                            key={asiento.idAsiento}
                            disabled={!asiento.disponible}
                            onClick={() =>
                              setAsientoSeleccionadoId(asiento.idAsiento)
                            }
                            style={{
                              width: "56px",
                              height: "48px",
                          ...
                          >
                            <div
                              style={{ fontSize: ".82rem", fontWeight: 700 }}
                            >
                              {asiento.columna}
                            </div>
                            <div style={{ fontSize: ".68rem" }}>
                              {asiento.disponible ? "Disp." : "Ocup."}
                            </div>
                          </button>
                        );
                      })}
        ...
        <div style={{ display: "grid", gap: "1rem" }}>
          ...
          >
            ...
            >
              <div>
                <strong style={{ color: "#f1f5f9" }}>Película:</strong>{" "}
                {pelicula?.pelicula.titulo ?? "—"}
              </div>
              <div>
                <strong style={{ color: "#f1f5f9" }}>Cine:</strong>{" "}
                {cineSeleccionado?.nombreCompania ?? "—"}
              </div>
              <div>
                <strong style={{ color: "#f1f5f9" }}>Sala:</strong>{" "}
                {salaSeleccionada?.nombre ?? "—"}
              </div>
              <div>
                <strong style={{ color: "#f1f5f9" }}>Función:</strong>{" "}
                {funcionSeleccionada
                  ? new Date(
                      funcionSeleccionada.fechaHoraInicio,
                    ).toLocaleString("es-ES")
                  : "—"}
              </div>
              <div>
                <strong style={{ color: "#f1f5f9" }}>Asiento:</strong>{" "}
                {asientoSeleccionado
                  ? `${asientoSeleccionado.fila}${asientoSeleccionado.columna}`
                  : "—"}
              </div>
              <div>
                <strong style={{ color: "#f1f5f9" }}>Monto:</strong> Q{" "}
                {funcionSeleccionada
                  ? Number(funcionSeleccionada.precio).toFixed(2)
                  : "0.00"}
              </div>
            </div>
          </div>
          ...
          <div style={{ color: "#94a3b8", fontSize: ".78rem" }}>
            Los asientos ocupados aparecen deshabilitados y no pueden
            seleccionarse.
          </div>
        </div>
      </div>
