...
const PeliculaCinesPage = () => {
  ...
  const [cineSeleccionadoId, setCineSeleccionadoId] = useState<number | null>(
    null,
  );
  const [salaSeleccionadaId, setSalaSeleccionadaId] = useState<number | null>(
    null,
  );
  const [commentForm, setCommentForm] =
    useState<CommentFormState>(emptyCommentForm);
  const [editingComment, setEditingComment] =
    useState<ComentarioSalaResponse | null>(null);
  ...
  const { data: bloqueoActivo } = useQuery({
    queryKey: ["bloqueo-activo-pelicula-cines", cineSeleccionadoId],
    queryFn: async () => {
      try {
        return await adsAdminCineService.obtenerBloqueoActivoPorCompania(
          cineSeleccionadoId as number, auth!.token
        );
      } catch (error: any) {
    ...
    enabled: !!cineSeleccionadoId,
    retry: false,
  });

  const mostrarAdsLaterales = !bloqueoActivo;

  const { data: funciones = [], isLoading: funcionesLoading } = useQuery({
    queryKey: ["pelicula-funciones-cine", id, cineSeleccionadoId],
    queryFn: () =>
      cinemaService.getFuncionesByPeliculaYCine(
        peliculaId,
        cineSeleccionadoId as number,
      ),
    enabled: !!id && !!cineSeleccionadoId,
  });

  const funcionesPorSala = useMemo(() => {
    return funciones.reduce<Record<number, FuncionResponse[]>>(
      (acc, funcion) => {
  ...
  const salasOrdenadas = useMemo(() => {
    return Object.entries(funcionesPorSala).map(([salaId, listaFunciones]) => ({
      ...
      funciones: [...listaFunciones].sort((a, b) =>
  ...
  const { data: comentariosSala = [], isLoading: comentariosSalaLoading } =
    useQuery({
      queryKey: ["comentarios-sala-publicos", salaSeleccionadaId],
      queryFn: () =>
        cinemaService.getComentariosBySala(salaSeleccionadaId as number),
  ...
  const createMutation = useMutation({
    mutationFn: (payload: ComentarioSalaRequest) =>
      cinemaUsuarioService.crearComentarioSala(
        salaSeleccionadaId as number,
        payload,
      ),
    onSuccess: (res) => {
      setMsg({
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: ComentarioSalaRequest) =>
      cinemaUsuarioService.actualizarComentarioSala(
        editingComment!.idComentarioSala,
        payload,
      ),
    onSuccess: (res) => {
      setMsg({
  ...
  const deleteMutation = useMutation({
    mutationFn: (comentario: ComentarioSalaResponse) =>
      cinemaUsuarioService.eliminarComentarioSala(
        comentario.idComentarioSala,
        auth!.idUsuario,
      ),
    onSuccess: (res) => {
      setMsg({
  ...
  const handleSeleccionarFuncion = (
    funcion: FuncionResponse,
    sala: SalaResponse,
  ) => {
  ...
  return (
    ...
    >
      ...
      <p
        style={{ fontSize: ".85rem", color: "#94a3b8", marginBottom: "1.5rem" }}
      >
        Selecciona un cine para ver sus salas, funciones disponibles y
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: mostrarAdsLaterales
      ...
      >
        {mostrarAdsLaterales && <AdsAside position="left" />}
        ...
        <div style={{ display: "grid", gap: "1rem" }}>
            ...
            {peliculaLoading ? (
              ...
              <>
                ...
                <div style={{ color: "#94a3b8", fontSize: ".85rem" }}>
                  Selecciona un cine, una función y luego abre la pantalla de
                  asientos.
                </div>
              </>
            )}
                ...
                {cines.map((cine) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "999px",
                      border:
                        cineSeleccionadoId === cine.idCompania
          ...
          >
            ...
            {funcionesLoading ? (
              ...
              <div style={{ color: "#94a3b8", fontSize: ".85rem" }}>
                No hay funciones registradas para el cine seleccionado.
              </div>
            ) : (
              <div style={{ display: "grid", gap: "1rem" }}>
                {salasOrdenadas.map(
                  ({ salaId, sala, funciones: funcionesSala }) => (
                    <div
                      key={salaId}
                      style={{
                        borderRadius: "14px",
                        ...
                        background:
                          salaSeleccionadaId === salaId
                            ? "rgba(37,99,235,0.16)"
                            : "rgba(30,64,175,0.12)",
                        border: `1px solid ${salaSeleccionadaId === salaId ? "rgba(96,165,250,0.4)" : "rgba(96,165,250,0.12)"}`,
                      }}
                    >
                      <div
                        style={{
                      ...
                      >
                        <div>
                          ...
                          <div style={{ color: "#94a3b8", fontSize: ".8rem" }}>
                            Capacidad {sala.capacidad} · {sala.filas} filas ·{" "}
                            {sala.columnas} columnas
                          </div>
                        </div>
                        <div
                          style={{
                            color: sala.visible ? "#86efac" : "#fca5a5",
                            fontSize: ".8rem",
                          }}
                        >
                      ...
                      <div style={{ display: "grid", gap: ".55rem" }}>
                        {funcionesSala.map((funcion) => (
                          <button
                            key={funcion.id}
                            onClick={() =>
                              handleSeleccionarFuncion(funcion, sala)
                            }
                            style={{
                              padding: ".75rem",
                              borderRadius: "10px",
                          ...
                          >
                            <div style={{ color: "#f1f5f9", fontWeight: 500 }}>
                              {new Date(funcion.fechaHoraInicio).toLocaleString(
                                "es-ES",
                                {
                                  hour: "2-digit",
                                  minute: "2-digit",
                            ...
                            >
                              Hasta{" "}
                              {new Date(funcion.fechaHoraFin).toLocaleString(
                                "es-ES",
                                {
                                  hour: "2-digit",
                                  minute: "2-digit",
                              ...
                              · Q {Number(funcion.precio).toFixed(2)}
                            </div>
                            <div
                              style={{
                                color: "var(--blue-glow)",
          ...
          >
            ...
            {!salaSeleccionada ? (
              <div style={{ color: "#94a3b8", fontSize: ".85rem" }}>
                Selecciona una sala para ver y comentar sus opiniones.
              </div>
            ) : (
              <>
                <div style={{ color: "#f1f5f9", marginBottom: ".85rem" }}>
                  Sala seleccionada:{" "}
                  <strong>{salaSeleccionada.sala.nombre}</strong>
                </div>
                ...
                {comentariosSalaLoading ? (
                  ...
                  <div style={{ color: "#94a3b8", fontSize: ".85rem" }}>
                    Todavía no hay comentarios para esta sala.
                  </div>
                ) : (
                        ...
                        >
                          ...
                          >
                            <div>
                              ...
                              <div
                                style={{ color: "#94a3b8", fontSize: ".76rem" }}
                              >
        ...
        {mostrarAdsLaterales && <AdsAside position="right" />}
      </div>
    </div>
  );
};
