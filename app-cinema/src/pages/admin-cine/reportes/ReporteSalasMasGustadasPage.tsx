...
import type { ReporteSalasMasGustadas } from "../../../types/CinemaCoreReports";
import type { SalaResponse } from "../../../types/CinemaCore.types";

const ReporteSalasMasGustadasPage: React.FC = () => {
  const { auth } = useAuth();
  ...
  const { data: reporteData, isLoading } = useQuery({
    ...
    queryFn: () =>
      reportesAdminCineService.reporteSalasMasGustadas(
        idCompania || 0,
        auth?.idUsuario || 0,
        {
  ...
  const reporte: ReporteSalasMasGustadas[] = Array.isArray(reporteData)
    ? reporteData
    : [];

  const totalCalificaciones = useMemo(() => {
    return reporte.reduce(
      (sum, sala) => sum + (sala.totalCalificaciones || 0),
      0,
    );
  }, [reporte]);
      ...
      >
        ...
        >
          ...
          <div style={{ color: "#93c5fd", fontSize: ".9rem" }}>
            Calificaciones: {totalCalificaciones} | Promedio: ⭐
            {promedioGeneral}
          </div>
        </div>
          ...
          <div style={{ overflowX: "auto" }}>
            ...
            >
              ...
              <tbody>
                {reporte
                  .sort(
                    (a, b) => b.promedioCalificacion - a.promedioCalificacion,
                  )
                  .slice(0, 5)
                  .map((sala) => (
                    <tr
            ...
            {reporte.length > 0 && (
              <div style={{ marginTop: "1.5rem" }}>
                ...
                <div style={{ display: "grid", gap: "1rem" }}>
                  {reporte.map((sala) =>
                    sala.calificaciones?.map((calificacion) => (
                      <div
                        key={`${sala.salaId}-${calificacion.idComentario}`}
                        style={{
                          padding: "1rem",
                          background: "rgba(15,23,42,0.55)",
                      ...
                      >
                        ...
                        >
                          ...
                          <div style={{ color: "#94a3b8", fontSize: ".8rem" }}>
                            ID Usuario: {calificacion.idUsuario}
                          </div>
                        </div>
                        <div
                        ...
                        >
                          ...
                          <span>
                            {new Date(calificacion.createdAt).toLocaleString()}
                          </span>
                        </div>
                      </div>
                    )),
...
export default ReporteSalasMasGustadasPage;
